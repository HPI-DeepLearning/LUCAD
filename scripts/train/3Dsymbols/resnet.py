...
def resnet(units, num_stages, filter_list, num_classes, image_shape, bottle_neck=True, bn_mom=0.9, workspace=256, memonger=False):
    ...
    (nchannel, depth, height, width) = image_shape
    if height <= 32:            # such as cifar10
        body = mx.sym.Convolution(data=data, num_filter=filter_list[0], kernel=(3, 3, 3), stride=(1,1,1), pad=(1, 1, 1),
                                  no_bias=True, name="conv0", workspace=workspace)
    elif height == 36:
        body = mx.sym.Convolution(data=data, num_filter=filter_list[0], kernel=(5, 5, 5), stride=(1,1,1),
                                  no_bias=True, name="conv0", workspace=workspace)
    ...
    # this kernel should be adapted for proper input size, the original kernel here is (7, 7)
    pool1 = mx.symbol.Pooling(data=relu1, global_pool=True, kernel=(4, 4, 4), pool_type='avg', name='pool1')
